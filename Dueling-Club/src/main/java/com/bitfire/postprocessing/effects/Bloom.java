...
public class Bloom extends PostProcessorEffect {
    ...
    public void applyCoef(float a) {
        super.applyCoef(a);
        setBloomIntesity(settings.bloomIntensity * a);
        setBlurAmount(settings.blurAmount * a);
        setThreshold(settings.bloomThreshold * a);
    ...
    public void render(final FrameBuffer src, final FrameBuffer dest) {
        ...
        if (isSaturateOnly()) {
            ...
            combine.setOutput(dest).setInput(texsrc, texsrc).render();
            return;
        }
