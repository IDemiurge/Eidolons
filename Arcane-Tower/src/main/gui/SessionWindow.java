...
import main.gui.sub.DirectionHeader;
import main.gui.sub.GoalPages;
import main.gui.sub.InfoEditPanel;
import main.gui.sub.SessionControlPanel;
import main.gui.sub.SessionTimer;
...
public class SessionWindow implements ActionListener {
	...
	public void refresh() {
		...
		directionBox.setModel(new DefaultComboBoxModel<Direction>(array));
	...
	private G_Panel initBoxPanel() {
		...
		for (Class c : comboBoxes) {
			...
			JComboBox<String> box = new JComboBox<String>(list.toArray(new String[list.size()]));
			// int x = 0, y = 0;
			// String pos = "pos " + x + " " + y;
			String pos = "";
			i++;
