...
import libgdx.map.editor.EditorManager;
import libgdx.map.obj.*;
import eidolons.macro.entity.party.MacroParty;
import eidolons.macro.map.Place;
import eidolons.macro.map.travel.MapWanderAi;
import libgdx.map.editor.EditorControlPanel;
...
public class MapObjStage extends Stage {
    ...
    public void act(float delta) {
        ...
        if (Flags.isMapEditor())
            pointsGroup.setVisible(EditorManager.getMode() == EditorControlPanel.MAP_EDITOR_MOUSE_MODE.POINT);
        if (!Flags.isMapEditor())
