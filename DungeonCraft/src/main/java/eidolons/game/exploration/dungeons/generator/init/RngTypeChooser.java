package eidolons.game.exploration.dungeons.generator.init;
...
public class RngTypeChooser {
    ...
    public static ObjType getType(
            GeneratorEnums.ROOM_CELL value,
...
//        }
    ...
    private static boolean isNeverRepeat(GeneratorEnums.ROOM_CELL value) {
        switch (value) {
            case ART_OBJ:
    ...
    private static GeneratorEnums.ROOM_CELL checkRandomCellResolves(GeneratorEnums.ROOM_CELL value) {
        if (Flags.isFullFastMode()) {
            if (TilesMaster.isPassable(value)) {
                return GeneratorEnums.ROOM_CELL.FLOOR;
            }
        }
        if (value.getRandomWeightMap() == null) {
            return value;
        }
        WeightMap<GeneratorEnums.ROOM_CELL> map = new WeightMap(value.getRandomWeightMap(), GeneratorEnums.ROOM_CELL.class);
        return map.getRandomByWeight();
    }


    public static boolean isCellTranslated(GeneratorEnums.ROOM_CELL value) {
        switch (value) {
            case VOID:
            case TRAP:
            case GUARDS:
