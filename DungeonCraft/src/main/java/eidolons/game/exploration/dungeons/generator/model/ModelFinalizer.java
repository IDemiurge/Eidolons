...
import eidolons.game.exploration.dungeons.generator.LevelDataMaker;
import eidolons.game.exploration.dungeons.generator.LevelValidator;
import eidolons.game.exploration.dungeons.generator.graph.GraphPath;
import eidolons.game.exploration.dungeons.generator.graph.LevelGraphNode;
...
import eidolons.game.exploration.dungeons.generator.level.ZoneCreator;
...
public class ModelFinalizer {
    ...
    public static void randomizeEdges(LevelModel model) {
        ...
        Map<Coordinates, GeneratorEnums.ROOM_CELL> modelMap = TileMapper.createTileMap(model).getMapModifiable();
        for (Room room : model.getRoomMap().values()) {
            LevelBlock block = model.getBlocks().get(room);
            Map<Coordinates, GeneratorEnums.ROOM_CELL> map = TileMapper.createTileMap(room).getMap();
            List<Coordinates> edgeCells = map.keySet().stream()
             .filter(c -> map.get(c) == GeneratorEnums.ROOM_CELL.INDESTRUCTIBLE)
             .filter(c -> TilesMaster.getAdjacentCount(modelMap, c, GeneratorEnums.ROOM_CELL.VOID) > 2)
             .collect(Collectors.toList());
            int last = 0;
            for (Coordinates edgeCell : edgeCells) {

                List<Coordinates> voidCells = TilesMaster.getAdjacentCells(modelMap, false, edgeCell, GeneratorEnums.ROOM_CELL.VOID);
                ...
                if (RandomWizard.chance(50 + last)) {
                    ...
                    block.getTileMap().getMapModifiable().put(c, GeneratorEnums.ROOM_CELL.INDESTRUCTIBLE);

                    if (edgeCell.x == c.x || edgeCell.y == c.y)
                        if (RandomWizard.chance(85)) {
                            block.getTileMap().getMapModifiable().put(edgeCell, GeneratorEnums.ROOM_CELL.FLOOR);
                            c = room.relative(edgeCell);
                            room.getCells()[c.x][c.y] = GeneratorEnums.ROOM_CELL.FLOOR.getSymbol();
    ...
    public void substituteRoomModels(LevelModel model) {
        ...
        for (Room room : model.getRoomMap().values()) {
            if (room.getUsedExits().size() == ExitMaster.getExitCount(room.getExitTemplate()))
                continue;
            GeneratorEnums.EXIT_TEMPLATE template = ExitMaster.getExitTemplateToLinks(room.getUsedExits().size(),
             room.getEntrance(), room.getType());

            if (template == GeneratorEnums.EXIT_TEMPLATE.ANGLE || template == GeneratorEnums.EXIT_TEMPLATE.THROUGH) {
                if (room.getUsedExits().get(0) == room.getEntrance().flip()) {
                    template = GeneratorEnums.EXIT_TEMPLATE.THROUGH;
                } else template = GeneratorEnums.EXIT_TEMPLATE.ANGLE;
            }
    ...
    private boolean tryConnectViaLink(Room room, Room room2, FACING_DIRECTION side) {
        Room link = builder.findFitting(room, GeneratorEnums.EXIT_TEMPLATE.THROUGH, ROOM_TYPE.CORRIDOR, side, true);
        {
            if (link != null) {
    ...
    private Set<Integer> getAligned(Room room, Room room2, boolean onXorY) {
        ...
        Map<Coordinates, GeneratorEnums.ROOM_CELL> map = TileMapper.createTileMap(room).getMap();
        Map<Coordinates, GeneratorEnums.ROOM_CELL> map2 = TileMapper.createTileMap(room2).getMap();
    ...
    private boolean connect(Room room, Room room2) {
        ...
        for (Coordinates pathCoordinate : path) {
            ...
            GeneratorEnums.ROOM_CELL cell = GeneratorEnums.ROOM_CELL.FLOOR;
    ...
    public void finalize(LevelModel model) {
        ...
        maxRooms =
         ...
         model.getData().getReqs().getIntValue(LevelDataMaker.LEVEL_REQUIREMENTS.maxRooms);
    ...
    private LevelGraphNode createAdditionalNode(LevelModel model, LevelData data) {
        ...
        GeneratorEnums.GRAPH_NODE_ATTRIBUTE attrs = null;
    ...
    private GeneratorEnums.EXIT_TEMPLATE getTemplate(Room room, LevelGraphNode node) {
