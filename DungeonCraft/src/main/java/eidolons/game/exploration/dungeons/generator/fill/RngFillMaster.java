...
public class RngFillMaster {
    ...
    public static void fill(LevelModel model, LevelData data) {
        ...
        WeightMap<GeneratorEnums.ROOM_CELL> weightMap = getMap(FILLER_TYPE.OVERLAYING_LIGHT_EMITTERS, data);
...
//            main.system.auxiliary.log.LogMaster.log(1, block + " has " + c1 + " BOUND TO " + c2);
    ...
    private static boolean canBeBoundFiller(GeneratorEnums.ROOM_CELL cell) {
        if (cell == null) {
            return false;
        }
        switch (cell) {
        ...
        for (LevelBlock block : model.getBlocks().values()) {
            ...
            List<Coordinates> filledCells = map.keySet().stream().
             filter(c -> block.getOriginalTileMap().
              getMap().get(c)== GeneratorEnums.ROOM_CELL.FLOOR
              && isClearable(map.get(c)) && TilesMaster.getAdjacentCount(map, c, GeneratorEnums.ROOM_CELL.DOOR) ==0).
             collect(Collectors.toList());
...
//            Coordinates c = filledCells.getVar(RandomWizard.getRandomIndex(filledCells));
    ...
    private static boolean isClearable(GeneratorEnums.ROOM_CELL cell) {
        return getClearChanceForCell(cell) != 0;
    }

    private static int getClearChanceForCell(
     GeneratorEnums.ROOM_CELL c) {
        if (c == null) {
            return 0;
        }
        switch ((c)) {
    ...
    private static void clearFill(LevelModel model, LevelBlock block, Coordinates c) {
        model.placeCell(c, block, GeneratorEnums.ROOM_CELL.FLOOR);
    }

    private static WeightMap<GeneratorEnums.ROOM_CELL> getMap(FILLER_TYPE type, LevelData data) {
        WeightMap<GeneratorEnums.ROOM_CELL> map = new WeightMap<>("", GeneratorEnums.ROOM_CELL.class);
        switch (type) {
            case LIGHT_EMITTERS:
                map.put(GeneratorEnums.ROOM_CELL.LIGHT_EMITTER, 1);
                break;
            case OVERLAYING_LIGHT_EMITTERS:
                map.put(GeneratorEnums.ROOM_CELL.WALL_WITH_LIGHT_OVERLAY, 1);
                break;
            case DECOR:
                map.put(GeneratorEnums.ROOM_CELL.ART_OBJ, 3);
                map.put(GeneratorEnums.ROOM_CELL.SPECIAL_ART_OBJ, 1);
                break;
            case OVERLAYING_DECOR:
                map.put(GeneratorEnums.ROOM_CELL.WALL_WITH_DECOR_OVERLAY, 1);
                break;
            case CONTAINER:
                map.put(GeneratorEnums.ROOM_CELL.CONTAINER, 3);
                map.put(GeneratorEnums.ROOM_CELL.SPECIAL_CONTAINER, 1);
                break;
            case SPECIAL_CONTAINER:
                map.put(GeneratorEnums.ROOM_CELL.SPECIAL_CONTAINER, 5);
                break;
            case GUARDS:
            case TRAPS:
                break;
            case DESTRUCTIBLE:
                map.put(GeneratorEnums.ROOM_CELL.DESTRUCTIBLE, 1);
                break;
        }
        return map;
    }
        ...
        for (LevelZone zone : model.getZones()) {
            ...
            for (int x = c.x; x < c.x + w; x++) {
                for (int y = c.y; y < c.y + h; y++) {
                    ...
                    if (!list.contains(c1))
                        map.put(c1, GeneratorEnums.ROOM_CELL.FLOOR);
                }
