package eidolons.game.exploration.dungeon.generator.test;
...
import eidolons.game.exploration.dungeon.generator.test.LevelStats.LEVEL_STAT;
...
public class LevelStats extends DataUnit<LEVEL_STAT> {
    ...
    public LevelStats(DungeonLevel level) {
        WeightMap<GeneratorEnums.EXIT_TEMPLATE> templateMap = new WeightMap();
        WeightMap<ROOM_TYPE> typeMap = new WeightMap();
        WeightMap<GeneratorEnums.ROOM_CELL> map = new WeightMap();
        for (Room room : level.getModel().getBlocks().keySet()) {
            GeneratorEnums.EXIT_TEMPLATE template = room.getExitTemplate();
            ROOM_TYPE type = room.getType();
            MapMaster.addToIntegerMap(templateMap, template, 1);
            MapMaster.addToIntegerMap(typeMap, type, 1);
            for (GeneratorEnums.ROOM_CELL roomCell : level.getModel().getBlocks().get(room).getTileMap().getMap().values()) {
