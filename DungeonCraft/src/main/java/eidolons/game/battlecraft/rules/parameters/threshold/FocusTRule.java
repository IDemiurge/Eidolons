...
import static main.content.enums.entity.UnitEnums.COUNTER.*;
...
public class FocusTRule extends ThresholdRule {
    ...
    @Override
    protected Map<UnitEnums.COUNTER, Dice> getCountersMapForThreshold(int n) {
        switch (n){
            case 3:
            case 2:
            case 1:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Stillness, Adrenaline, Energy).
                        values(new Dice(1, d4), new Dice(1, d4)).build();
            case -1:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Haze).
                        values(new Dice(2, d4)).build();
            case -2:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Haze, Concussion).
