...
public class ToughnessTRule extends ThresholdRule {
    ...
    protected Map<UnitEnums.COUNTER, Dice> getCountersMapForThreshold(int n) {
        switch (n){
            case 0:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Pain, Fatigue).
                        values(new Dice(1, d4), new Dice(1, d4)).build();
            case 1:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Pain, Bleeding).
                        values(new Dice(1, d4), new Dice(1, d4)).build();
            case 2:
                return  new MapBuilder<UnitEnums.COUNTER, Dice>().keys(Adrenaline, Bleeding).
