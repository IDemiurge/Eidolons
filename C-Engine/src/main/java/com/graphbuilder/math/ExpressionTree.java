 ...
 * function name. The first character found that is an operator, whitespace,
 * open bracket, close bracket or comma marks the end of the name. If the first
 ...
 * <li>Constants can be represented as functions that take no parameters. E.g.
 * pi() or e()</li>
 * <li>Round brackets '(' and ')' are the only brackets that have special
 * meaning.</li>
 * <li>The brackets in the expression must balance otherwise a
 * ExpressionParseException is thrown.</li>
 * <li>An ExpressionParseException is thrown in all cases where the expression
 * string is invalid.</li>
 * <li>All terms must be separated by an operator. E.g. 2x is <b>not</b> valid,
 * but 2*x is.</li>
 * <li>In cases where simplification is possible, simplification is <b>not</b>
 * done. E.g. 2^4 is <b>not</b> simplified to 16.</li>
 * <li>Computerized scientific notation is supported for values. E.g. 3.125e-4</li>
 * <li>Scoped negations are not permitted. E.g. -(a) is <b>not</b> valid, but
 * -1*(a) is.</li>
 * </ul>
 *
...
public class ExpressionTree {
    ...
    /**
     * Returns an expression-tree that represents the expression string. Returns
    ...
    private static Expression build(String s, int indexErrorOffset) {
        return build(false, s, indexErrorOffset);
    }

    private static Expression build(boolean autoresolved, String s, int indexErrorOffset) {
        ...
        boolean term = true; // indicates a term should come next, not an
        // operator
        boolean signed = false; // indicates if the current term has been signed
        boolean negate = false; // indicates if the sign of the current term is
        // negated
