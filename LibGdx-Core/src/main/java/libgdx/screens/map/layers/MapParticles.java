package libgdx.screens.map.layers;
...
import libgdx.screens.map.editor.EditorParticleMaster;
...
public class MapParticles extends MapTimedLayer<EmitterActor> {
    ...
    public void init() {
        ...
        for (MAP_EMITTER_GROUP sub : MAP_EMITTER_GROUP.values()) {
            ...
            if (sub.points != null)
                for (MapMoveLayers.MAP_POINTS point : sub.points) {
    ...
    public enum MAP_EMITTER_GROUP {
        ...
        float[] intensity;
        Color[] colors;
        MapMoveLayers.MAP_POINTS[] points;
        int distance;
        int number;
        float speed;
        MapMoveLayers.MOVE_DIRECTION direction;
        ...
        public void setPoints(MapMoveLayers.MAP_POINTS... points) {
            this.points = points;
        }
    }
