...
public class GuiVisualEffects extends GroupX {
    ...
    private void initEmitters(VisualEnums.VFX_TEMPLATE template, DAY_TIME time) {
        if (emitters != null) {
    ...
    private int getEmitterCount(VisualEnums.VFX_TEMPLATE template, DAY_TIME time) {
        return 3;
    }

    private WeightMap<GenericEnums.VFX> getEmittersWeightMap(VisualEnums.VFX_TEMPLATE template, boolean night) {
    ...
    private void createEmitters(boolean bottom, GenericEnums.VFX preset, int gap) {
        ...
        for (int i = 0; i < GdxMaster.getWidth(); i += gap) {
            ...
            if (!RandomWizard.chance(chance*2)) {
                try {
                    preset_ = ShardEnums.getEmitters(null , VisualEnums.SHARD_SIZE.NORMAL)[0];
                } catch (Exception e) {
                    continue;
                }
            }
