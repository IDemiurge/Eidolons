    ...
    public void init() {
        ...
        for (int x = x1 - 1; x - 1 < x2; x++) {
            for (int y = y1 - 1; y - 1 < y2; y++) {
                ...
                VisualEnums.SHARD_SIZE size = builder.chooseSize(x, y, direction);
                if (size == null) {
                    //                just empty    continue;
                }
                VisualEnums.SHARD_TYPE type = builder.getType(x, y);

                VisualEnums.SHARD_OVERLAY overlay = new EnumMaster<VisualEnums.SHARD_OVERLAY>().
