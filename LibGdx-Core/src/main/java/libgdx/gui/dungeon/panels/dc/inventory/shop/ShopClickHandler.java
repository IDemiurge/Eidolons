...
import libgdx.gui.dungeon.panels.headquarters.datasource.HqDataMasterDirect;
import eidolons.game.battlecraft.logic.meta.universal.shop.Shop;
import eidolons.game.battlecraft.logic.meta.universal.shop.ShopTransactions;
...
public class ShopClickHandler extends ContainerClickHandler {
    ...
    public boolean cellClicked(VisualEnums.CELL_TYPE cell_type, int clickCount, boolean rightClick, boolean altClick, Entity cellContents, boolean ctrlClick) {
        ...
        if (clickCount > 1 || rightClick) {
            if (cell_type == VisualEnums.CELL_TYPE.CONTAINER) {
        ...
        if (ctrlClick) {
            if (cell_type == VisualEnums.CELL_TYPE.CONTAINER) {
                ...
                if (cell_type == VisualEnums.CELL_TYPE.INVENTORY) {
                    operation = OPERATIONS.SELL;
                } else
        ...
        if (operation == OPERATIONS.DROP) {
            if (cell_type == VisualEnums.CELL_TYPE.CONTAINER) {
                operation = OPERATIONS.BUY;
            } else if (cell_type == VisualEnums.CELL_TYPE.STASH) {
    ...
    @Override
    protected OPERATIONS getInvOperation(VisualEnums.CELL_TYPE cell_type,
