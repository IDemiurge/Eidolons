...
import static data.C3Enums.CodeTaskStatus;
import static data.C3Enums.TaskCategory;
...
public class C3TaskManager extends C3Handler {
    ...
    public List<C3_Task> getTasksFor(List<String> names) {
        ...
        for (String name : names) {
            // List objs =
            //         new MapMaster<C3_Task>().searchLayered(taskTextMap, obj -> framework.task.category == obj);
    ...
    private C3_Task createTask(TaskCategory category, boolean randomSub, boolean randomTask, boolean customTask) {
        ...
        if (customTask) {
            ...
            taskString = DialogMaster.inputText("Input framework.task text");
        } else {
 ...
 */
    ...
    private String chooseTask(List<String> strings) {
        Object option = DialogMaster.getChosenOption("Select framework.task", strings.toArray());
        if (option != null) {
